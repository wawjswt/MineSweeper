...
const entryId = "src/adapters/miniprogram/runtime.js";
...
function collectModule(moduleId) {
  ...
  source = source.replace(exportFromPattern, () => {
    throw new Error(`Unsupported export-from syntax in ${moduleId}`);
  });
...
function buildBundle() {
  ...
  for (const moduleId of moduleOrder) {
    ...
    lines.push(indent(modules.get(moduleId).source, 2));
    lines.push("};");
  }

  lines.push(`module.exports = __require(${JSON.stringify(entryId)});`);
  return `${lines.join("\n")}\n`;
}
